...
class Pix2PixModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        # 存储选定的类别
        self.selected_class = opt.selected_class
        self.num_classes = opt.label_nc + (1 if opt.contain_dontcare_label else 0)
    ...
    def preprocess_input(self, data):
        ...
        if self.opt.label_nc == 1:
            ...
            # 如果指定了特定类别，创建只包含该类别的one-hot编码
            if self.selected_class >= 0 and self.selected_class < nc:
                single_class_mask = self.FloatTensor(bs, numnodes, nc, h, w).zero_()
                single_class_mask[:, :, self.selected_class] = 1.0
                input_semantics = single_class_mask
